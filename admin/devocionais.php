...
<style>
    ...
    /* Customização do Quill Editor para Dark Mode (Sacred Minimalist) */
    ...
    /* Micro-movimento de toque premium spring active */
    .interactive-scale {
...
<main class="max-w-4xl mx-auto px-4 sm:px-6 py-8 mb-32 space-y-8 animate-fade-in" id="espiritualidade-container">
    ...
    <div class="relative overflow-hidden rounded-[2px] bg-white dark:bg-[#18191D] text-gray-800 dark:text-white p-8 shadow-sm dark:shadow-xl border border-gray-100 dark:border-[#26272B] reveal-item">
        <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-[2px] bg-[#2E7EED]/10 dark:bg-[#2E7EED]/20 border border-[#2E7EED]/20 dark:border-[#2E7EED]/30 text-[#2E7EED] text-xs font-bold uppercase tracking-wider mb-3">
                ...
                <p class="text-gray-500 dark:text-gray-400 mt-2 max-w-xl text-sm font-body">Mantenha a fé activa compartilhando reflexões da palavra de Deus e intercedendo uns pelos outros.</p>
            </div>
        </div>
    </div>

    <!-- TABS Flutuantes Bento Premium -->
    <div class="flex items-center gap-1 p-1 bg-gray-100 dark:bg-[#121316] rounded-[2px] w-fit shadow-sm border border-transparent dark:border-[#26272B] reveal-item">
        <button class="px-5 py-2.5 rounded-[2px] text-sm font-extrabold transition-all duration-200 interactive-scale cursor-pointer tab-btn active" onclick="switchTab('word')" id="btn-tab-word">📖 Palavra</button>
        <button class="px-5 py-2.5 rounded-[2px] text-sm font-extrabold transition-all duration-200 interactive-scale cursor-pointer tab-btn text-gray-500 dark:text-gray-400" onclick="switchTab('prayer')" id="btn-tab-prayer">🙏 Oração</button>
    </div>

    <!-- CONTEÚDO: PALAVRA (DEVOCIONAIS) -->
    <div id="tab-word" class="tab-content block space-y-6">
        <!-- Busca e Filtros Bento -->
        <div class="bg-white dark:bg-[#18191D] p-6 rounded-[2px] shadow-sm border border-gray-100 dark:border-[#26272B] space-y-4 reveal-item">
            <div class="relative w-full">
                ...
                <input 
                    ...
                    class="w-full h-12 pl-12 pr-4 bg-gray-50 dark:bg-[#121316] border border-gray-100 dark:border-[#26272B] rounded-[2px] text-sm focus:outline-none focus:border-[#2E7EED] dark:focus:border-[#2E7EED] focus:ring-1 focus:ring-[#2E7EED] transition-all placeholder-gray-400 text-gray-800 dark:text-white font-medium"
            ...
            <div class="flex flex-wrap items-center gap-3 pt-1">
                <span class="text-xs font-bold text-gray-400 dark:text-gray-550 uppercase tracking-wider pl-1">Filtrar:</span>
                
                <div class="flex gap-1.5 p-0.5 bg-gray-50 dark:bg-[#121316] rounded-[2px] border border-gray-100 dark:border-[#26272B]">
                    <a href="?read_status=all&author=<?= $filterAuthor ?>&tag=<?= $filterTag ?>&search=<?= urlencode($search) ?>&tab=word" 
                       class="px-3.5 py-1.5 rounded-[2px] text-xs font-bold transition-all <?= $filterRead === 'all' ? 'bg-[#2E7EED]/10 text-[#2E7EED]' : 'text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-white' ?>">
                        Todas
                    </a>
                    <a href="?read_status=unread&author=<?= $filterAuthor ?>&tag=<?= $filterTag ?>&search=<?= urlencode($search) ?>&tab=word" 
                       class="px-3.5 py-1.5 rounded-[2px] text-xs font-bold transition-all <?= $filterRead === 'unread' ? 'bg-[#2E7EED]/10 text-[#2E7EED]' : 'text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-white' ?>">
                        Não Lidas
                    </a>
                    <a href="?read_status=read&author=<?= $filterAuthor ?>&tag=<?= $filterTag ?>&search=<?= urlencode($search) ?>&tab=word" 
                       class="px-3.5 py-1.5 rounded-[2px] text-xs font-bold transition-all <?= $filterRead === 'read' ? 'bg-[#2E7EED]/10 text-[#2E7EED]' : 'text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-white' ?>">
                        Lidas
                    </a>
                </div>

                <span class="w-[1px] h-4 bg-gray-200 dark:bg-[#26272B]"></span> <!-- Autor Select -->
                <select onchange="window.location.href='?author='+this.value+'&read_status=<?= $filterRead ?>&tag=<?= $filterTag ?>&search=<?= urlencode($search) ?>&tab=word'" 
                        class="h-8 pl-3 pr-8 bg-gray-50 dark:bg-[#121316] border border-gray-100 dark:border-[#26272B] rounded-[2px] text-xs font-bold text-gray-600 dark:text-gray-300 focus:outline-none focus:border-[#2E7EED] cursor-pointer appearance-none bg-[url('data:image/svg+xml,%3Csvg xmlns=%27http://www.w3.org/2000/svg%27 width=%2712%27 height=%2712%27 viewBox=%270 0 24 24%27 fill=%27none%27 stroke=%27%2364748b%27 stroke-width=%272%27%3E%3Cpolyline points=%276 9 12 15 18 9%27%3E%3C/polyline%3E%3C/svg%3E')] bg-no-repeat bg-[position:right_10px_center]">
                ...
                <a href="?tab=word" class="h-8 px-3.5 rounded-[2px] text-xs font-bold bg-red-50 dark:bg-red-950/20 hover:bg-red-100 dark:hover:bg-red-900/30 text-red-500 dark:text-red-400 flex items-center gap-1 transition-colors">
        ...
        <div class="space-y-4">
            <?php if (count($devotionals) > 0): ?>
                ...
                ?>
                <div class="devotional-card bg-white dark:bg-[#18191D] rounded-[2px] border border-gray-100 dark:border-[#26272B] shadow-sm p-6 space-y-4 reveal-item <?= $dev['is_read'] ? 'opacity-85' : 'border-l-2 border-l-[#2E7EED]' ?>" id="dev-<?= $dev['id'] ?>">
                    <!-- Header -->
                    <div class="flex items-center justify-between gap-4">
                        <div class="flex items-center gap-3">
                            <?php if ($authorAvatar): ?>
                                <img src="<?= htmlspecialchars($authorAvatar) ?>" class="w-10 h-10 rounded-[2px] object-cover border border-gray-100 dark:border-[#26272B] shadow-sm shrink-0" alt="">
                            <?php else: ?>
                                <div class="w-10 h-10 rounded-[2px] bg-[#2E7EED]/10 text-[#2E7EED] font-extrabold flex items-center justify-center text-sm border border-transparent shadow-sm shrink-0"><?= strtoupper(substr($authorName, 0, 1)) ?></div>
                            <?php endif; ?>
                            ...
                            <div class="flex flex-col">
                                <span class="text-sm font-bold text-gray-800 dark:text-gray-100 flex items-center flex-wrap gap-1.5 leading-tight">
                                    ...
                                    <?php if (!empty($dev['series_title'])): ?>
                                        <span class="text-xs font-bold text-gray-400 dark:text-gray-550">em</span>
                                        <span class="inline-flex items-center px-2 py-0.5 rounded-[2px] text-[10px] font-extrabold" style="background: <?= $dev['series_color'] ?: '#2E7EED' ?>12; color: <?= $dev['series_color'] ?: '#2E7EED' ?>;">
                                            📚 <?= htmlspecialchars($dev['series_title']) ?>
                                        </span>
                                    <?php endif; ?>
                                </span>
                                <div class="flex items-center gap-1.5 text-[11px] text-gray-400 dark:text-gray-550 font-bold uppercase tracking-wider mt-0.5">
                                    <span><?= date('d/m \à\s H:i', strtotime($dev['created_at'])) ?></span>
                                    <?php if (!$dev['is_read']): ?>
                                        <span class="w-1.5 h-1.5 rounded-[2px] bg-emerald-500 animate-pulse"></span>
                        ...
                        <div class="shrink-0">
                             <form method="POST" onsubmit="return confirm('Excluir este devocional permanentemente?');" class="m-0">
                                ...
                                <button type="submit" class="w-8 h-8 rounded-[2px] bg-gray-50 dark:bg-[#121316] hover:bg-red-50 dark:hover:bg-red-950/20 text-gray-400 hover:text-red-500 flex items-center justify-center transition-colors cursor-pointer border border-gray-100 dark:border-[#26272B]">
                    ...
                    <div class="space-y-3">
                        ...
                        ?>
                        <div class="aspect-video w-full rounded-[2px] overflow-hidden shadow-sm border border-gray-100 dark:border-[#26272B] bg-gray-50 dark:bg-[#121316]">
                        ...
                        <div class="pt-2">
                            <a href="<?= htmlspecialchars($dev['media_url']) ?>" target="_blank" class="flex items-center gap-3 p-4 bg-gray-50 dark:bg-[#121316] hover:bg-[#2E7EED]/5 dark:hover:bg-[#2E7EED]/10 border border-gray-100 dark:border-[#26272B] hover:border-[#2E7EED]/20 rounded-[2px] text-slate-800 dark:text-gray-200 transition-all duration-200 cursor-pointer">
                                <div class="w-10 h-10 rounded-[2px] bg-white dark:bg-[#18191D] text-[#2E7EED] border border-gray-150 dark:border-[#26272B] flex items-center justify-center shrink-0">
                    ...
                    <div class="flex flex-wrap gap-1.5 pt-2">
                        <?php foreach ($tags as $tag): ?>
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-[2px] bg-slate-50 dark:bg-[#121316] border border-slate-100 dark:border-[#26272B] text-slate-500 dark:text-gray-400 text-xs font-bold">
                    ...
                    <!-- Footer Actions -->
                    <div class="flex items-center gap-3 pt-3 border-t border-gray-50 dark:border-[#26272B]">
                        <button class="inline-flex items-center gap-1.5 text-xs font-bold text-gray-500 dark:text-gray-400 hover:text-[#2E7EED] bg-gray-50 dark:bg-[#121316] px-3.5 py-2 rounded-[2px] transition-all interactive-scale cursor-pointer" onclick="toggleComments('comments-<?= $dev['id'] ?>', event)">
                            <i data-lucide="message-square" class="w-4 h-4"></i>
                            <span>Comentários (<?= count($comments) ?>)</span>
                        </button>
                        <button class="inline-flex items-center gap-1.5 text-xs font-bold text-gray-500 dark:text-gray-400 hover:text-emerald-500 bg-gray-50 dark:bg-[#121316] px-3.5 py-2 rounded-[2px] transition-all interactive-scale cursor-pointer" onclick="shareDevotional(<?= $dev['id'] ?>, '<?= addslashes($dev['title']) ?>', event)">
                    ...
                    <!-- Comments Section -->
                    <div id="comments-<?= $dev['id'] ?>" class="hidden pt-4 border-t border-gray-50 dark:border-[#26272B] space-y-3">
                        ...
                        <div class="space-y-3 max-h-[300px] overflow-y-auto pr-1">
                            ...
                            ?>
                            <div class="flex gap-3 bg-gray-50/50 dark:bg-[#121316]/40 p-3 rounded-[2px] border border-gray-100 dark:border-[#26272B]">
                                <?php if ($commentAvatar): ?>
                                    <img src="<?= htmlspecialchars($commentAvatar) ?>" class="w-8 h-8 rounded-[2px] object-cover shrink-0 border border-gray-150 dark:border-white/5 shadow-sm" alt="">
                                <?php else: ?>
                                    <div class="w-8 h-8 rounded-[2px] bg-slate-200 dark:bg-[#121316] text-slate-500 dark:text-gray-400 font-extrabold flex items-center justify-center text-xs shrink-0"><?= strtoupper(substr($comment['author_name'], 0, 1)) ?></div>
                                <?php endif; ?>
                                ...
                                <div class="shrink-0">
                                    <form method="POST" onsubmit="return confirm('Excluir este comentário?');" class="m-0">
                                        ...
                                        <button type="submit" class="w-6 h-6 rounded-[2px] bg-white dark:bg-[#121316] hover:bg-red-50 dark:hover:bg-red-950/20 text-gray-400 hover:text-red-500 flex items-center justify-center border border-gray-150 dark:border-[#26272B] transition-colors cursor-pointer">
                        ...
                        <form method="POST" class="flex gap-2 pt-2">
                            ...
                            <input type="text" name="comment" class="flex-1 h-10 px-4 bg-gray-50 dark:bg-[#121316] border border-gray-150 dark:border-[#26272B] rounded-[2px] text-xs focus:outline-none focus:border-[#2E7EED] focus:ring-1 focus:ring-[#2E7EED] transition-all placeholder-gray-400 text-gray-800 dark:text-white font-medium" placeholder="Escreva uma mensagem..." required>
                            <button type="submit" class="w-10 h-10 rounded-[2px] bg-[#2E7EED] hover:bg-[#1A6FD6] text-white flex items-center justify-center shrink-0 shadow-sm active:scale-[0.97] transition-all cursor-pointer">
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <div class="bg-gray-50 dark:bg-[#18191D] border border-dashed border-gray-200 dark:border-[#26272B] rounded-[2px] p-12 text-center reveal-item">
                    <div class="w-16 h-16 rounded-[2px] bg-[#2E7EED]/10 text-[#2E7EED] flex items-center justify-center mx-auto mb-4 border border-transparent">
    ...
    <div id="tab-prayer" class="tab-content hidden space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <?php if ($prayerTableExists && count($prayers) > 0): ?>
                ...
                ?>
                <div class="prayer-card bg-white dark:bg-[#18191D] p-6 rounded-[2px] border border-gray-100 dark:border-[#26272B] shadow-sm flex flex-col justify-between transition-all duration-200 active:scale-[0.97] will-change-transform reveal-item <?= $prayer['is_urgent'] ? 'border-l-2 border-l-red-800 dark:border-l-red-650' : '' ?>" id="prayer-<?= $prayer['id'] ?>">
                    <div class="space-y-4 flex-1">
                        <!-- Top Info -->
                        <div class="flex items-start justify-between gap-4">
                            <div class="flex items-center gap-3">
                                <?php if ($authorAvatar): ?>
                                    <img src="<?= $authorAvatar ?>" class="w-9 h-9 rounded-[2px] object-cover border border-gray-100 dark:border-[#26272B] shadow-sm shrink-0">
                                <?php else: ?>
                                    <div class="w-9 h-9 rounded-[2px] bg-red-950/20 text-red-500 font-extrabold flex items-center justify-center text-xs shrink-0 border border-[#26272B]">
                                ...
                                <div class="flex flex-col">
                                    ...
                                    <span class="text-[9px] text-gray-400 dark:text-gray-550 font-bold uppercase tracking-wider mt-0.5"><?= date('d/m \à\s H:i', strtotime($prayer['created_at'])) ?></span>
                                </div>
                            </div>
                            
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-[2px] text-[10px] font-extrabold bg-gray-50 dark:bg-[#121316] border border-gray-100 dark:border-[#26272B] text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                                <?php
                                    $cats = [
                        ...
                        <!-- Content -->
                        <div class="space-y-2">
                            <h3 class="text-base font-extrabold text-gray-850 dark:text-white flex items-center gap-1.5 leading-tight font-display">
                                <?php if($prayer['is_urgent']): ?>
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-[2px] bg-red-950/30 border border-red-900/40 text-red-400 text-[9px] font-black uppercase tracking-widest shrink-0 animate-pulse">🔥 Urgente</span>
                                <?php endif; ?>
                    ...
                    <!-- Footer / Intercessão -->
                    <div class="flex items-center justify-between gap-3 pt-4 mt-4 border-t border-gray-50 dark:border-[#26272B] shrink-0">
                        <button onclick="toggleIntercessionStatus(<?= $prayer['id'] ?>, this); event.stopPropagation();" class="inline-flex items-center gap-2 text-xs font-bold text-gray-500 dark:text-gray-400 hover:text-red-500 bg-gray-50 dark:bg-[#121316] hover:bg-red-50 dark:hover:bg-red-950/20 border border-gray-100 dark:border-[#26272B] px-4 py-2.5 rounded-[2px] transition-all duration-200 active:scale-[0.97] will-change-transform cursor-pointer <?= $prayer['is_interceded'] ? 'active bg-red-950/20 border-red-900/40 text-red-400' : '' ?>">
                        ...
                        <form method="POST" class="m-0" onclick="event.stopPropagation()">
                            ...
                            <button type="submit" class="inline-flex items-center gap-1 px-3 py-2 bg-emerald-950/20 border border-emerald-900/30 text-emerald-400 font-bold text-xs rounded-[2px] active:scale-[0.97] will-change-transform transition-all cursor-pointer">
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <div class="col-span-1 md:col-span-2 bg-gray-50 dark:bg-[#18191D] border border-dashed border-gray-200 dark:border-[#26272B] rounded-[2px] p-12 text-center reveal-item">
                    <div class="w-16 h-16 rounded-[2px] bg-red-950/20 text-red-500 border border-[#26272B] flex items-center justify-center mx-auto mb-4">
...
<div class="fixed bottom-6 right-6 flex flex-col sm:flex-row gap-3 z-40">
    <button onclick="openCreatePrayerModal()" class="inline-flex items-center justify-center gap-2 px-5 py-3.5 rounded-[2px] bg-amber-600 hover:bg-amber-700 text-white font-bold text-sm shadow-lg border border-amber-500/20 active:scale-[0.97] will-change-transform transition-all duration-200 cursor-pointer" id="fab-prayer" style="display: none;">
        <i data-lucide="heart" class="w-4 h-4"></i> Novo Pedido
    </button>
    <button onclick="openCreateModal()" class="inline-flex items-center justify-center gap-2 px-5 py-3.5 rounded-[2px] bg-[#2E7EED] hover:bg-[#1A6FD6] text-white font-bold text-sm shadow-lg border border-blue-500/20 active:scale-[0.97] will-change-transform transition-all duration-200 cursor-pointer" id="fab-word">
...
<div id="devotionalModal" class="hidden fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4 transition-opacity duration-300 opacity-0">
    <div id="devotionalModalContent" class="bg-white dark:bg-[#18191D] w-full max-w-2xl rounded-[2px] p-6 shadow-2xl transition-all transform scale-95 duration-300 max-h-[90vh] overflow-y-auto border border-gray-100 dark:border-[#26272B]">
        <div class="flex items-center justify-between mb-6 pb-4 border-b border-gray-100 dark:border-[#26272B]">
            <h2 class="text-xl font-bold text-gray-850 dark:text-white flex items-center gap-2">📖 Novo Devocional</h2>
            <button onclick="closeModalForce('devotionalModal')" class="w-8 h-8 rounded-[2px] bg-gray-50 hover:bg-gray-100 dark:bg-[#121316] dark:hover:bg-[#18191D] flex items-center justify-center text-gray-500 dark:text-gray-400 transition-colors cursor-pointer border border-transparent hover:border-gray-200 dark:hover:border-[#26272B]">
        ...
        <form method="POST" id="devotionalForm" onsubmit="return prepareSubmit()" class="space-y-4">
            ...
            <div class="flex flex-col gap-1.5">
                ...
                <input type="text" name="title" required placeholder="Ex: A paz que excede todo entendimento" class="w-full h-11 px-4 bg-gray-50 dark:bg-[#121316] border border-gray-100 dark:border-[#26272B] rounded-[2px] text-sm focus:outline-none focus:border-[#2E7EED] dark:focus:border-[#2E7EED] focus:ring-1 focus:ring-[#2E7EED] transition-all placeholder-gray-400 dark:placeholder-gray-500 text-gray-800 dark:text-white font-semibold">
            </div>
            
            <div class="flex flex-col gap-1.5">
                <label class="text-xs font-bold text-gray-400 dark:text-gray-550 uppercase tracking-wider pl-1">Mensagem/Conteúdo</label>
                <div class="border border-gray-100 dark:border-[#26272B] rounded-[2px] overflow-hidden shadow-sm">
                    <div id="editor" style="height: 180px; background: white;" class="font-sans text-sm dark:bg-[#121316]"></div>
                </div>
            </div>
            
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="flex flex-col gap-1.5">
                    <label class="text-xs font-bold text-gray-400 dark:text-gray-550 uppercase tracking-wider pl-1">Tipo de Mídia</label>
                    <select name="media_type" onchange="toggleMediaInput(this.value)" class="w-full h-11 px-3 bg-gray-50 hover:bg-gray-100 dark:bg-[#121316] dark:hover:bg-[#18191D] border border-gray-100 dark:border-[#26272B] rounded-[2px] text-sm font-semibold text-gray-700 dark:text-gray-300 focus:outline-none focus:border-[#2E7EED] transition-all">
                ...
                <div id="mediaUrlGroup" class="hidden flex flex-col gap-1.5 animate-fade-in">
                    <label class="text-xs font-bold text-gray-400 dark:text-gray-550 uppercase tracking-wider pl-1">URL da Mídia</label>
                    <input type="url" name="media_url" placeholder="https://youtube.com/watch?v=..." class="w-full h-11 px-4 bg-gray-50 dark:bg-[#121316] border border-gray-100 dark:border-[#26272B] rounded-[2px] text-sm focus:outline-none focus:border-[#2E7EED] dark:focus:border-[#2E7EED] focus:ring-1 focus:ring-[#2E7EED] transition-all placeholder-gray-400 dark:placeholder-gray-550 text-gray-800 dark:text-white font-semibold">
                </div>
            </div>
            
            <!-- Tags Selection Multi-Select -->
            <div class="flex flex-col gap-1.5">
                <label class="text-xs font-bold text-gray-400 dark:text-gray-550 uppercase tracking-wider pl-1">Tags (Assuntos)</label>
                <div class="relative w-full custom-dropdown">
                    <div onclick="toggleTagDropdownForm()" id="tagDropdownBtn" class="w-full h-11 px-4 bg-gray-50 hover:bg-gray-100 dark:bg-[#121316] dark:hover:bg-[#18191D] border border-gray-100 dark:border-[#26272B] rounded-[2px] text-sm font-semibold text-gray-700 dark:text-gray-300 flex items-center justify-between gap-2 transition-colors cursor-pointer">
                        <span class="dark:text-gray-400">Selecionar Tags...</span>
                        <i data-lucide="chevron-down" class="w-4 h-4 text-gray-500"></i>
                    </div>
                    <div id="tagDropdownList" class="hidden absolute top-full left-0 right-0 mt-2 bg-white dark:bg-[#121316] border border-gray-100 dark:border-[#26272B] rounded-[2px] shadow-xl z-50 flex flex-col max-h-[180px] overflow-y-auto divide-y divide-gray-50 dark:divide-[#26272B]">
                        <?php foreach ($allTags as $tag): ?>
                        <div onclick="toggleTagCheckboxForm(<?= $tag['id'] ?>, event)" class="px-4 py-2.5 hover:bg-gray-50 dark:hover:bg-[#18191D] text-sm text-gray-700 dark:text-gray-300 flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="tags[]" value="<?= $tag['id'] ?>" class="tag-form-checkbox w-4 h-4 rounded-[2px] border-gray-300 dark:border-[#26272B] dark:bg-[#121316] text-[#2E7EED] focus:ring-[#2E7EED]">
            ...
            <div class="flex gap-3 pt-4 border-t border-gray-100 dark:border-[#26272B]">
                <button type="button" onclick="closeModalForce('devotionalModal')" class="flex-1 py-3 bg-gray-50 hover:bg-gray-100 dark:bg-[#121316] dark:hover:bg-[#18191D] border border-gray-100 dark:border-[#26272B] text-gray-700 dark:text-gray-300 font-bold text-sm rounded-[2px] active:scale-[0.97] will-change-transform transition-all cursor-pointer">Cancelar</button>
                <button type="submit" class="flex-[2] py-3 bg-[#2E7EED] hover:bg-[#1A6FD6] text-white font-bold text-sm rounded-[2px] shadow-sm active:scale-[0.97] will-change-transform transitio<!-- Modal Create Prayer -->
<div id="prayerModal" class="hidden fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4 transition-opacity duration-300 opacity-0">
    <div id="prayerModalContent" class="bg-white dark:bg-[#18191D] w-full max-w-md rounded-[2px] p-6 shadow-2xl transition-all transform scale-95 duration-300 max-h-[85vh] overflow-y-auto border border-gray-100 dark:border-[#26272B]">
        <div class="flex items-center justify-between mb-6 pb-4 border-b border-gray-100 dark:border-[#26272B]">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-[2px] bg-red-950/20 text-red-500 flex items-center justify-center shrink-0 border border-[#26272B]">
            ...
            <button onclick="closeModalForce('prayerModal')" class="w-8 h-8 rounded-[2px] bg-gray-50 hover:bg-gray-100 dark:bg-[#121316] dark:hover:bg-[#18191D] flex items-center justify-center text-gray-500 dark:text-gray-400 transition-colors cursor-pointer border border-transparent hover:border-gray-200 dark:hover:border-[#26272B]">
                <i data-lucide="x" class="w-4 h-4"></i>
            </button>
        </div>
        ...
        <form method="POST" class="space-y-4">
            <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token']) ?>">
            <div class="flex flex-col gap-1.5">
                <label class="text-xs font-bold text-gray-400 dark:text-gray-550 uppercase tracking-wider pl-1">Título do Pedido</label>
                <input type="text" name="title" required placeholder="Ex: Oração pela saúde do meu pai" class="w-full h-11 px-4 bg-gray-50 dark:bg-[#121316] border border-gray-100 dark:border-[#26272B] rounded-[2px] text-sm focus:outline-none focus:border-[#2E7EED] dark:focus:border-[#2E7EED] focus:ring-1 focus:ring-[#2E7EED] transition-all placeholder-gray-400 dark:placeholder-gray-550 text-gray-800 dark:text-white font-semibold">
            </div>
            
            <div class="flex flex-col gap-1.5">
                <label class="text-xs font-bold text-gray-400 dark:text-gray-550 uppercase tracking-wider pl-1">Descrição detalhada</label>
                <textarea name="description" rows="4" placeholder="Adicione detalhes adicionais se desejar..." class="w-full border-gray-100 dark:border-[#26272B] bg-gray-50 dark:bg-[#121316] border rounded-[2px] text-sm focus:outline-none focus:border-[#2E7EED] dark:focus:border-[#2E7EED] focus:ring-1 focus:ring-[#2E7EED] transition-all placeholder-gray-400 dark:placeholder-gray-550 text-gray-855 dark:text-gray-200 p-4 resize-none font-body leading-relaxed"></textarea>
            </div>
            
            <div class="flex flex-col gap-1.5">
                <label class="text-xs font-bold text-gray-400 dark:text-gray-555 uppercase tracking-wider pl-1">Categoria</label>
                <div class="grid grid-cols-3 gap-2">
                    <label class="cat-option border border-gray-100 dark:border-[#26272B] rounded-[2px] p-2.5 flex flex-col items-center justify-center gap-1.5 cursor-pointer bg-white dark:bg-[#121316] transition-all text-center select-none text-gray-500 dark:text-gray-400 active:scale-[0.97] will-change-transform" onclick="selectFormCategory(this)">
                    ...
                    <label class="cat-option border border-[#2E7EED] dark:border-[#2E7EED] rounded-[2px] p-2.5 flex flex-col items-center justify-center gap-1.5 cursor-pointer bg-[#2E7EED]/5 dark:bg-[#2E7EED]/10 transition-all text-center select-none text-[#2E7EED] active:scale-[0.97] will-change-transform" onclick="selectFormCategory(this)">
            ...
            <div class="flex gap-4 pt-1 pb-2">
                <label class="flex items-center gap-2 cursor-pointer select-none">
                    <input type="checkbox" name="is_urgent" class="w-4.5 h-4.5 rounded-[2px] border-gray-300 dark:border-[#26272B] dark:bg-[#121316] text-red-650 focus:ring-red-650">
                    <span class="text-xs font-bold text-gray-600 dark:text-gray-300 uppercase tracking-wider">🔥 Urgente</span>
                </label>
                <label class="flex items-center gap-2 cursor-pointer select-none">
                    <input type="checkbox" name="is_anonymous" class="w-4.5 h-4.5 rounded-[2px] border-gray-300 dark:border-[#26272B] dark:bg-[#121316] text-slate-500 focus:ring-slate-500">
            ...
            <div class="flex gap-3 pt-4 border-t border-gray-100 dark:border-[#26272B]">
                ...
                <button type="submit" class="flex-[2] py-3 bg-[#2E7EED] hover:bg-[#1A6FD6] text-white font-bold text-sm rounded-[2px] shadow-sm active:scale-[0.97] will-change-transform transition-all cursor-pointer">Enviar Pedido</button>
            </div>
        </form>
    </div>
</div>
